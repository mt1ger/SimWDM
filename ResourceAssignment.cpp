...
void ResourceAssignment::check_availability_source (unsigned int predecessor, unsigned int successor, CircuitRequest * circuitRequest) {
	...
	for (int j = 0; j < network->NumofWavelengths; j++) {
		if (network->Wavelengths[predecessor][successor][j] == false) {
...
void ResourceAssignment::handle_requests (CircuitRequest * circuitRequest) {
	...
	for (int i = 1; i < CircuitRoute.size (); i++) {
		...
		for (int k = 0; k < network->NumofWavelengths; k++) {
			cout << network->Wavelengths[CircuitRoute[i - 1]][CircuitRoute[i]][k] << ' ';
		}
		cout << endl;
	}
	#endif
	...
	if (SourceAvailableWL.empty ()) AvailableFlag = false;
		...
		for (int i = 0; i < SourceAvailableWL.size (); i++) {
			...
			for (int k = 2; k < CircuitRoute.size (); k++) {
				...
				if (network->Wavelengths[CircuitRoute[k - 1]][CircuitRoute[k]][SourceAvailableWL[i][0]] == true) {
					AvailableFlag = false;
					break;
				}
			}
			if (AvailableFlag == true) {
				for (int n = 1; n < CircuitRoute.size (); n++) {
					network->Wavelengths[CircuitRoute[n - 1]][CircuitRoute[n]][SourceAvailableWL[i][0]] = true;	
				}
				wavelength = SourceAvailableWL[i][0];
				break;
			}
		}
	}
	...
	if (AvailableFlag == false) {
		...
		circuitRelease = new CircuitRelease (circuitRequest->EventID, CircuitRoute, wavelength, circuitRequest->StartTime + circuitRequest->Duration);
...
void ResourceAssignment::handle_releases (CircuitRelease * circuitRelease) {
	...
	for (int i = 1; i < circuitRelease->CircuitRoute.size (); i++) {
		...
		for (int k = 0; k < network->NumofWavelengths; k++) {
			cout << network->Wavelengths[circuitRelease->CircuitRoute[i - 1]][circuitRelease->CircuitRoute[i]][k] << ' ';
		}
		cout << endl;
	}
	#endif
	...
	for (int i = 1; i < circuitRelease->CircuitRoute.size (); i++) {
		network->Wavelengths[circuitRelease->CircuitRoute[i - 1]][circuitRelease->CircuitRoute[i]][circuitRelease->Wavelength] = false;	
	}
	
	network->NumofDoneRequests++;
	cout << "++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++" << endl;
	cout << "Release Event: " << circuitRelease->EventID << "\tTime: " << circuitRelease->EventTime << endl;
	cout << "Wavelength: " << circuitRelease->Wavelength << endl;
	cout << "++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++" << endl << endl;
